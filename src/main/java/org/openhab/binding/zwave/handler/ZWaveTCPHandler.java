...
public class ZWaveTCPHandler extends ZWaveControllerHandler {
    ...
    // public ZWaveTCPHandler(Bridge bridge, TranslationProvider translationProvider) {
                    ...
                    try {
                        ...
                        if (!getThing().getStatus().equals(ThingStatus.OFFLINE)) {
                            updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.OFFLINE.COMMUNICATION_ERROR,
                                    ZWaveBindingConstants.OFFLINE_TCP_EXISTS); // EDIT TO INCLUDE tcp_hostname
                        }
                    } catch (IOException e) {
                        if (!getThing().getStatus().equals(ThingStatus.OFFLINE)) {
                            updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.OFFLINE.COMMUNICATION_ERROR,
                                    ZWaveBindingConstants.OFFLINE_TCP_CONNECTION); // EDIT TO INCLUDE tcp_hostname
                        }
                    }
                }
    ...
    public void onConnectionLost() {
        ...
        if (!getThing().getStatus().equals(ThingStatus.OFFLINE)) {
            updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.OFFLINE.COMMUNICATION_ERROR,
                    ZWaveBindingConstants.OFFLINE_TCP_DISCONNECTED); // EDIT TO INCLUDE tcp_hostname
        }
    }
